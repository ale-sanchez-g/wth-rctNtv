...
test.beforeAll(async() => {
  ...
  Batch = new BatchInfo({name: `ReactNative test ${runnerName}`});
  ...
  if (USE_ULTRAFAST_GRID) {
    ...
    // Add 1 real iOs device for cross-browser testing in the Ultrafast Grid.
    Config.addBrowser({
        iosDeviceInfo: {
          deviceName: IosDeviceName.iPhone_14_Pro_Max,
